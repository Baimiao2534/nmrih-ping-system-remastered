...
float minFloat(float x, float y) { return (x < y) ? x : y; }
...
public void OnPluginStart()
{
	...
	cvTokensPerSecond	   = CreateConVar("sm_ping_cooldown_tokens_per_second", "0.8", "Tokens added to the bucket per second");
	cvBucketSize		   = CreateConVar("sm_ping_cooldown_bucket_size", "5", "Number of command tokens that fit in the cooldown bucket");
	...
	cvLifetime			   = CreateConVar("sm_ping_lifetime", "9.5", "The lifetime of player pings in seconds", _, true, 1.0);
	cvRange				   = CreateConVar("sm_ping_range", "7500", "The maximum reach of the player ping trace in game units");
	...
	cvCircleRadius		   = CreateConVar("sm_ping_circle_radius", "8", "Radius of the ping circle");
	cvAllowPlayers		   = CreateConVar("sm_ping_players", "1", "Whether pings can target other players");
	...
	CreateConVar("nmrih_ping_version", PLUGIN_VERSION, PLUGIN_DESCRIPTION,
...
Action Cmd_Ping(int client, int args)
{
	if (!client)
	{
		CReplyToCommand(client, "%t", "In Game Only");
...
void PingWorld(float pos[3], float normal[3], int issuer, int duration)
{
	...
	int cvCircleSegments = GetRandomInt(3 , 9);
	DrawCircleOnSurface(pos, cvCircleRadius.FloatValue, cvCircleSegments, normal, duration, g_PingColor[issuer]);
...
void BeginDrawWorldTextAll(float pos[3], int issuer, int duration, int moveParent = -1)
{
	...
	if (!showDistance)
	{
		...
		CreateTimer(float(duration), Timer_DeleteWorldText, pingID);
	}
	else
	{
...
public bool TraceFilter_Ping(int entity, int contentMask, int ignore)
{
	...
	if (!cvAllowNPCs.BoolValue)
	{
		...
		return StrContains(classname, "npc_nmrih_") != 0;
	}
...
bool CheckCanUsePing(int client)
{
	...
	g_Tokens[cooldownIndex]		= minFloat(g_Tokens[cooldownIndex] + tokensToAdd, cvBucketSize.FloatValue);
